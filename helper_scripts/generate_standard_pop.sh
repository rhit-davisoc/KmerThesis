...
# rm -r $standard_folder/fasta_files/*
...
# mkdir $output_pop_folder/kmer_counts/ &&
# mkdir $output_pop_folder/kmer_counts/tmp/ &&
# mkdir $output_pop_folder/kmer_counts/tmp/$test_name"/" &&
# mkdir $output_pop_folder/kmer_counts/$test_name"/" &&

# for i in $(seq 0 $((2*num_indv-1)))
# do
#     KMC3/kmc -t$cpus -k$km -ci$min_count $read_file_path"n"$i"_R1.fastq" $output_pop_folder/kmer_counts/tmp/$test_name"/n"$i"_R1" .&&

#     KMC3/kmc -t$cpus -k$km -ci$min_count $read_file_path"n"$i"_R2.fastq" $output_pop_folder/kmer_counts/tmp/$test_name"/n"$i"_R2" . &&

#     ./KMC3/bin/kmc_tools -t$cpus simple $output_pop_folder/kmer_counts/tmp/$test_name"/n"$i"_R1" $output_pop_folder/kmer_counts/tmp/$test_name"/n"$i"_R2" union $output_pop_folder/kmer_counts/tmp/$test_name/n$i &&

#     ./KMC3/bin/kmc_tools -t$cpus transform $output_pop_folder/kmer_counts/tmp/$test_name/n$i  dump $output_pop_folder/kmer_counts/$test_name/n$i.txt
# done
...
#         if [ $i -lt $j ]
#         then
#             ./KMC3/bin/kmc_tools -t$cpus simple $output_pop_folder/kmer_counts/tmp/$test_name"/n"$i $output_pop_folder/kmer_counts/tmp/$test_name"/n"$j intersect $output_pop_folder/kmer_counts/tmp/$test_name/n$i"_n"$j &&

#             ./KMC3/bin/kmc_tools -t$cpus transform $output_pop_folder/kmer_counts/tmp/$test_name/n$i"_n"$j  dump $output_pop_folder/kmer_counts/$test_name/n$i"_inter_n$j".txt
#         fi
#     done
# done